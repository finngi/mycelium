    ...
    def convert(src: Path, dst: Path):
        with open(src) as f, open(dst, "w") as out:
            for line in f:
                ...
                prompt_text = (
                    prompt.replace("{name}", obj["input"]) if prompt else obj["input"]
                )
...
def _maybe_push_to_hf(adapter_dir: Path, trial_id: str) -> str | None:
    ...
    try:
        ...
        print(
            f"[WARN] HF publish failed: {e} -> falling back to local adapter path",
            file=sys.stderr,
        )
        return None
...
def train(trial_manifest: TrialManifest) -> TrainerResult:
    ...
    if base_model is None:
        raise ValueError(
            "mlx_lora trainer needs base_model (from-scratch MLX training isn't implemented)"
        )
    ...
    try:
        print(
            f"[INFO] loading {base_model} (cold start may take a while to download weights)",
            file=sys.stderr,
        )
        ...
        if not use_chat_template:
            print(
                f"[WARN] {base_model} has no chat_template -> training/eval will concatenate raw prompt+completion text",
                file=sys.stderr,
            )
        ...
        if mask_prompt and not use_chat_template:
            ...
            print(
                "[WARN] mask_prompt is only supported for chat/completion datasets -> ignoring for this text-format run",
                file=sys.stderr,
            )
            mask_prompt = False
        ...
        mx.random.seed(
            seed
        )  # lora_run() only seeds np.random (batch order), not mx.random (LoRA init/dropout)
        ...
        print(
            f"[INFO] training: method={method} iters={iters} seed={seed}",
            file=sys.stderr,
        )
        lora_run(args)
        ...
        for row in val_rows:
            prompt_text = (
                prompt.replace("{name}", row["input"]) if prompt else row["input"]
            )
            if use_chat_template:
            ...
            pred_text = generate(
                model, tokenizer, formatted, max_tokens=768, sampler=sampler
            )
        ...
        weights_uri = _maybe_push_to_hf(adapter_dir, trial_manifest["id"]) or str(
            adapter_dir
        )
    finally:
        heartbeat.stop()
