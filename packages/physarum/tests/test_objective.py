...
from physarum.objective import build_recipe, make_objective, resolve_metric, suggest
...
def test_resolve_metric_slash_key_is_plain_not_a_path():
    # 'val/field_f1' is record_eval's split-prefix convention -- the '/' is
...
def test_make_objective_excludes_infeasible_trial_from_best_even_with_best_raw_value():
    sweep = Sweep(
    ...
    # direction must match sweep.objective's "maximize", or study.best_trial
    ...
    study.optimize(make_objective(sweep, fake_trainer), n_trials=2)
...
def test_make_objective_min_constraint_direction():
    sweep = Sweep(
        **SWEEP_WITH_CONSTRAINT_KWARGS,
...
def test_make_objective_missing_constrained_metric_counts_infeasible():
    sweep = Sweep(
        **SWEEP_WITH_CONSTRAINT_KWARGS,
...
def test_make_objective_carries_observables_onto_the_trial():
