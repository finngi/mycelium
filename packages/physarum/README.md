...
planned and run through the exact same `Producer` contract
...
A `Sweep` is a recipe template plus a search space over its `hparams`
hyperparameters, a goal metric, and a sampler:

```yaml
name: extract-lora-sweep-1
template:
  name: extract-lora-sweep-1   # overridden per-trial; still required
  task: extract
...
`runtime: cpu` skips oyster/mlx entirely: physarum's own trafilatura
extraction producer runs in-process, no model or gradient step involved. See
`experiments/htmlmd-trafilatura/sweep.yaml` for a full grid-search example.

## v1 scope

In-process only: `study.optimize()` calls the resolved `Producer` directly
